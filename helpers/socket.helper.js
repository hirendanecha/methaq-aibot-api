  ...
  io.sockets.on("connection", (socket) => {
    ...
    socket.on('archive-chat', async (params, cb) => {
      try {
        ...
        const newMessage = new MessageModel(mess)
        const final = await newMessage.save();
        const updatedChat = await ChatModel.findOneAndUpdate({ _id: chatId }, { latestMessage: final?._id, adminId: null, isHuman: false, status: "archived", department: null }, { new: true })?.populate("adminId customerId").lean();
