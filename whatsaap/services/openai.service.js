...
const { UserContextService } = require("../services/user-context.service");
...
Remember to keep the interactions human-like, personable, and infused with creativity while maintaining a professional demeanor. Your primary objective is to assist the user effectively while making the conversation enjoyable.`;
      ...
      const userContext = await userContextService.saveAndFetchContext(
        userInput,
        "user",
        userID
      );
      this.logger.info(userContext);
      ...
      await userContextService.saveToContext(aiResponse, "assistant", userID);
